...
def writeFilenameExtractedCoordinates(filename, lig_resname, atom_Ids, pathFolder, writeLigandTrajectory, constants, writeCA):
    ...
    if writeLigandTrajectory:
        outputFilename = os.path.join(pathFolder, "{}.dat".format(os.path.splitext(os.path.basename(filename))[0]))
        with open(outputFilename, 'w') as f:
            f.write("\nENDMDL\n".join(allCoordinates))
    ...
    path = os.path.join(pathFolder, "extractedCoordinates/coord_{}.dat".format(os.path.splitext(os.path.basename(filename))[0]))
    writeToFile(coords, os.path.abspath(path))
...
def copyTrajectories(traj_names, destFolderTempletized, folderName):
    for inputTrajectory in traj_names:
        ...
        shutil.copyfile(inputTrajectory, os.path.join("allTrajs", os.path.basename(inputTrajectory.replace("coord", "traj"))))
...
def main(folder_name=".", atom_Ids="", lig_resname="", numtotalSteps=0, enforceSequential_run=0, writeLigandTrajectory=False, setNumber=0, protein_CA=0, non_Repeat=False):
    constants = Constants()
    ...
    if enforceSequential_run:
        ...
        folders = [folderWithTrajs,] 

    for folder_it in folders:
        pathFolder = folder_it
